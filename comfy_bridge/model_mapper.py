...
class ModelMapper:
    ...
    async def initialize(self, comfy_url: str):
        ...
        print(f"Initialized with {len(self.available_models)} models. Default: {self.default_model}")
    ...
    def _build_model_map(self):
        ...
        for key, default_filename in self.DEFAULT_MODEL_MAP.items():
            for filename in lower_model_filenames:
                if key.replace("_", "").replace("-", "") in filename.replace("_", "").replace("-", ""):
    ...
    def get_model_filename(self, horde_model_name: str) -> str:
        return (
            ...
            or next((v for k, v in self.model_map.items() if horde_model_name.lower() in k.lower()), None)
