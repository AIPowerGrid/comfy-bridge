            ...
            if positive_node.get("class_type") == "CLIPTextEncode":
                if "inputs" in positive_node and "text" in positive_node["inputs"]:
                    ...
                    if "POSITIVE_PROMPT_PLACEHOLDER" in text:
                        ...
                        logger.error(
                            "❌ PROBLEM: Positive prompt node has NEGATIVE placeholder!"
                        )
                    else:
                ...
                logger.error(
                    f"Node {positive_node_id} is not a CLIPTextEncode node: {positive_node.get('class_type')}"
                )
        else:
            ...
            if negative_node.get("class_type") == "CLIPTextEncode":
                if "inputs" in negative_node and "text" in negative_node["inputs"]:
                    ...
                    if "NEGATIVE_PROMPT_PLACEHOLDER" in text:
                        ...
                        logger.error(
                            "❌ PROBLEM: Negative prompt node has POSITIVE placeholder!"
                        )
                    else:
                ...
                logger.error(
                    f"Node {negative_node_id} is not a CLIPTextEncode node: {negative_node.get('class_type')}"
                )
        else:
...
def analyze_web_format(workflow: Dict[str, Any]) -> None:
    ...
    if positive_node:
        if positive_node.get("type") == "CLIPTextEncode":
            if (
                "widgets_values" in positive_node
                and len(positive_node["widgets_values"]) > 0
            ):
            ...
            logger.error(
                f"Positive node is not a CLIPTextEncode node: {positive_node.get('type')}"
            )
    else:
    ...
    if negative_node:
        if negative_node.get("type") == "CLIPTextEncode":
            if (
                "widgets_values" in negative_node
                and len(negative_node["widgets_values"]) > 0
            ):
            ...
            logger.error(
                f"Negative node is not a CLIPTextEncode node: {negative_node.get('type')}"
            )
    else:
...
def fix_workflow(workflow_path: str) -> None:
    ...
    if isinstance(data, dict) and "nodes" not in data:
        ...
        if ksampler_node and "inputs" in ksampler_node:
            if (
                "positive" in ksampler_node["inputs"]
                and "negative" in ksampler_node["inputs"]
            ):
                # Swap positive and negative connections
                temp = ksampler_node["inputs"]["positive"]
                ksampler_node["inputs"]["positive"] = ksampler_node["inputs"][
                    "negative"
                ]
                ksampler_node["inputs"]["negative"] = temp
                logger.info(
                    f"Swapped positive and negative connections in KSampler node {ksampler_id}"
                )
    ...
    # For web format
        ...
        logger.error(
            "Fixing web format workflows not implemented - export to API format first"
        )
