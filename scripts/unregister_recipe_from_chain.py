#!/usr/bin/env python3
"""
Unregister Recipe/Workflow from RecipeVault on Blockchain

This script unregisters workflows/recipes from the RecipeVault contract
on Base Mainnet through the Grid Diamond proxy by setting isPublic=false
and canCreateNFTs=false.
...
Usage:
    python scripts/unregister_recipe_from_chain.py --recipe-id <id>
    python scripts/unregister_recipe_from_chain.py --recipe-root <root_hash>
    python scripts/unregister_recipe_from_chain.py --recipe-id <id> --dry-run
...
def main():
    """Main function."""
    import argparse
    parser = argparse.ArgumentParser(description='Unregister workflow/recipe from RecipeVault')
    parser.add_argument('--recipe-id', '-i', type=str, help='Recipe ID to unregister (as string)')
    parser.add_argument('--recipe-root', '-r', help='Recipe root hash to unregister')
    parser.add_argument('--dry-run', action='store_true', 
                       help='Show what would be unregistered without actually unregistering')
    args = parser.parse_args()
    
    print('\n' + '=' * 60)
    print('AIPG RecipeVault - Unregister Recipe from Blockchain')
    if args.dry_run:
        print('                    [DRY RUN MODE]')
    print('=' * 60)
    print(f'RPC: {RPC_URL}')
    ...
    # Check for private key (only required for actual unregistration)
    if not PRIVATE_KEY and not args.dry_run:
        print('\n[ERROR] PRIVATE_KEY environment variable required for unregistration')
    ...
    try:
        ...
        if not recipe_info["isPublic"] and not recipe_info["canCreateNFTs"]:
            print('\n   [INFO] Recipe is already unregistered (isPublic=false, canCreateNFTs=false)')
            return
    ...
    print('\n[INFO] Unregistration will set:')
    print('   - canCreateNFTs: false')
    print('   - isPublic: false')
    print('\n   This will effectively hide the recipe from public use.')
    ...
    if args.dry_run:
        ...
        if args.recipe_id:
            print(f'   python scripts/unregister_recipe_from_chain.py --recipe-id {args.recipe_id}')
        else:
            print(f'   python scripts/unregister_recipe_from_chain.py --recipe-root {args.recipe_root}')
        print('')
    ...
    try:
        ...
        if not updated_info["isPublic"] and not updated_info["canCreateNFTs"]:
            print('   [SUCCESS] Recipe successfully unregistered!')
            print(f'   - canCreateNFTs: {updated_info["canCreateNFTs"]}')
            print(f'   - isPublic: {updated_info["isPublic"]}')
        else:
            print('   [WARNING] Recipe update may not have taken effect')
            print(f'   - canCreateNFTs: {updated_info["canCreateNFTs"]}')
            print(f'   - isPublic: {updated_info["isPublic"]}')
    ...
    print('\n' + '=' * 60)
    print('   Recipe unregistered successfully!')
    print('=' * 60 + '\n')
